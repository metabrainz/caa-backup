...
def import_data(batch_size, force, incremental):
    ...
    importer = CAAImporter(pg_conn_string=pg_conn_string, db_path=db_path, batch_size=batch_size)
...
def download(threads, batch_size, monitor_port):
    ...
    try:
        downloader = CAADownloader(
            db_path=db_path, images_dir=images_dir, batch_size=batch_size, download_threads=threads
        )
